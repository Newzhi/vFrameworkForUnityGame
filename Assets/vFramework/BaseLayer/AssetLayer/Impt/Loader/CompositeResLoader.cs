...
{
    /// <summary>
    /// 按顺序尝试多个 IResLoader；支持前缀 res:// / yoo:// / addr:// 直达指定后端。
    /// </summary>
    public sealed class CompositeResLoader : IResLoader
    {
        ...
        private IResLoader ResolveLoader(string location, bool onlyCheck)
        {
            ...
            if (location.StartsWith(YooAssetResLoader.Prefix, StringComparison.OrdinalIgnoreCase))
            {
                return Find<YooAssetResLoader>() ?? FindFallback(typeof(YooAssetResLoader));
            }
