...
2. **依赖下沉**：第三方库（UniTask、YooAsset、Protobuf 等）仅在底层引用，通过 **Interface + Impt（实现/适配）** 向上暴露能力。
...
- 底层加载走 **YooAsset**（或 Addressables）Adapter，Adapter 仅在 BaseFramework / AssetLayer Impt 中引用。
- 逻辑层调用：`SceneMgr.LoadSingleAsync("scene_battle")`，不直接 `YooAssets.LoadSceneAsync`。
...
| `BaseLayer` | 全局 Manager | BaseFramework、YooAsset 等 |
...
1. GameRoot + 模块注册中心  
2. BaseEventSys（已有）  
3. AssetLayer 三 Manager 接口 + Resources/YooAsset 最小 Adapter  
4. NetMgr 消息分发  
